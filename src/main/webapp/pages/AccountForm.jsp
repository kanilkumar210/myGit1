...
<body>
	<sql:setDataSource var="dbsource" driver="com.mysql.cj.jdbc.Driver"
		url="jdbc:mysql://localhost:3306/spring_project" user="root"
		password="Anilkarre8@" />

	<sql:query dataSource="${dbsource}" var="result">
            SELECT * from spring_project.corporate_setup where is_active=1;
        </sql:query>
	<div class="container">

		<h2 style="color: blue; text-align: center;">Add Account Details</h2>
		<a href="getAccountDetails"><button class="abc3">Back</button></a>
		<div class="row">
			<!-- <a href="view1.jsp" style="text-align: right;">View All Records</a> -->
			<hr>

			<div class="col-md-4 offset-4" modelAttribute="accBean">
				<form action="addAccount">

					<label class="form-group">Corporate Name</label> <select
						name="corporate.corporate_id" style="width: 155px"
						required="required" class="form-control">
						<c:forEach items="${result.rows}" var="row">
							<option value="${row.corporate_id}">${row.corporate_name}</option>
						</c:forEach>
					</select> <label class="form-group">Account Number:</label> <input
						type="number" placeholder="Account Number" name="accountNumber"
						required="required" class="form-control" /> <label
						class="form-group">Account Name:</label> <input type="text"
						placeholder="Account Name" name="accountName" required="required"
					...
					<select name="branch" style="width: 155px" required="required"
						...
						placeholder="Balance" type="number" name="availableBalance"
